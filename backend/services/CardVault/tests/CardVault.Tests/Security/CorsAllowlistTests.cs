...
/// <summary>
/// SEC-6: CardVault CORS policy must block non-allowlisted origins and echo back
...
public class CorsAllowlistTests
{
    ...
    // Note: WithWebHostBuilder returns the base WebApplicationFactory<T>, so we keep a reference
    // to the derived factory (which controls ConfigureWebHost) and apply the extra setting via
    // the builder delegate that runs AFTER ConfigureWebHost.
