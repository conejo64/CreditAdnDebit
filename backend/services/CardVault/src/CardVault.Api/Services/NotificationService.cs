...
public sealed class NotificationService
{
    ...
    public async Task<int> DispatchPendingDeliveriesAsync(int take, CancellationToken ct)
    {
        ...
        {
            delivery.Attempts++;
            delivery.LastAttemptOn = DateTimeOffset.UtcNow;
            delivery.Status = NotificationDeliveryStatus.Sent;
            delivery.DeliveredOn = DateTimeOffset.UtcNow;
            delivery.ProviderReference = $"sim-{delivery.Channel.ToString().ToLowerInvariant()}-{delivery.Id:N}";
            delivery.LastError = null;
            sent++;

            _db.OutboxMessages.Add(new OutboxMessageEntity
            {
                Topic = NotificationDeliveredTopic,
                ...
                PayloadJson = JsonSerializer.Serialize(new
                {
                    ...
                    notificationType = delivery.Notification.Type.ToString(),
                    severity = delivery.Notification.Severity.ToString(),
                    traceId = delivery.Notification.TraceId
                })
            });

            await _audit.WriteAsync("cardvault.notification.delivered", new
            {
                notificationId = delivery.NotificationId,
                deliveryId = delivery.Id,
                channel = delivery.Channel.ToString(),
                ...
                providerReference = delivery.ProviderReference,
                traceId = delivery.Notification.TraceId
            }, delivery.NotificationId.ToString("N"), delivery.Notification.TraceId, ct);

            await _pciAudit.PublishAsync("pci.notification.delivered", delivery.NotificationId.ToString("N"), new
            {
                notificationId = delivery.NotificationId,
                deliveryId = delivery.Id,
                channel = delivery.Channel.ToString(),
