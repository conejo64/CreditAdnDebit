...
namespace CardVault.Api.Features.Auth.Commands;
...
public class LoginCommandHandler : IRequestHandler<LoginCommand, IResult>
{
    ...
    public LoginCommandHandler(UserManager<AppUser> users, TokenService tokens, IdentityAppDbContext idDb, Microsoft.Extensions.Configuration.IConfiguration config)
    {
        _users = users;
        _tokens = tokens;
        _idDb = idDb;
    ...
    public async Task<IResult> Handle(LoginCommand request, CancellationToken cancellationToken)
    {
        ...
        var hash = TokenService.HashRefreshToken(refresh);
...
public class MfaVerifyCommandHandler : IRequestHandler<MfaVerifyCommand, IResult>
{
    ...
    public MfaVerifyCommandHandler(UserManager<AppUser> users, TokenService tokens, IdentityAppDbContext idDb, Microsoft.Extensions.Configuration.IConfiguration config)
    {
        _users = users;
        _tokens = tokens;
        _idDb = idDb;
...
public class RefreshTokenCommandHandler : IRequestHandler<RefreshTokenCommand, IResult>
{
    ...
    public RefreshTokenCommandHandler(IdentityAppDbContext idDb, UserManager<AppUser> users, TokenService tokens, Microsoft.Extensions.Configuration.IConfiguration config)
    {
        _idDb = idDb;
        _users = users;
        _tokens = tokens;
...
public class GetCurrentUserQueryHandler : IRequestHandler<GetCurrentUserQuery, IResult>
{
    ...
    public GetCurrentUserQueryHandler(UserManager<AppUser> users, TokenService tokens)
    {
        _users = users;
        _tokens = tokens;
    }
