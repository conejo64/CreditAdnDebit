...
**`ICardVaultDbContext` was evaluated and REJECTED (see ADR-6 below).** The EF entity types live in `Infrastructure.Persistence` and the relocated handlers/services consume them directly, so a `DbContext` port does not remove the `Application → Persistence` dependency — it only abstracts the context while the entity types still force the reference. Per ARCH-5 minimalism ("no speculative ports"), CardVault gets NO new port: services move wholesale and inject `CardVaultDbContext` concretely. CardVault notification/audit emission likewise stay concrete. No port has an impl inside its own Application project (ARCH-5, ARCH-10).
...
| Hazard | Handling |
|--------|----------|
| **Reference cycle** (`Application` needs entity types in `Persistence`, which already had a dead `→ Application` ref) | **ADR-6 (revised at CV-S2+S3 apply):** the EF entities live in `Persistence` and are consumed directly, so a `DbContext` port does NOT break the dependency. Resolution: `Application → Persistence` one-way; REMOVE the dead (unused-in-source) `Persistence → Application` reference. No port. Documented, time-bounded compromise (ARCH-DEP-2) revisited in Ola 4+. `dotnet build` must prove no cycle. |
...
- **ADR-8 — CV-S2+S3 must also relocate `Contracts/` and introduce 2 behavior ports (discovered during CV-S2+S3 apply).** The relocated services/handlers do NOT move cleanly: they (a) return/accept the DTOs in `CardVault.Api/Contracts/**` (e.g. `WalletTokenView`, `RegisterWalletTokenRequest`, `ThreeDsChallengeView`), and (b) depend on two concrete Api services — `Vault/TokenVaultService` and `Pci/PciAuditPublisher`. Leaving these in `Api` forces `Application → Api` (forbidden, and a cycle). **Decision:** (1) move `Contracts/**` into `CardVault.Application` (they are application data contracts; Api controllers reference them from Application — `Api → Application`, allowed); purely-HTTP envelopes with Api-only deps may stay in Api and are reported. (2) Introduce minimal ports in Application — `IPanVault` (for `TokenVaultService`) and `IPciAuditPublisher` (for `PciAuditPublisher`); the **implementations stay in `Api`** and are wired via DI (`Api → Application` lets Api implement Application interfaces). This is the LEGITIMATE port use the proposal anticipated (IAuditWriter-style) — these are *behavior* dependencies, unlike the rejected `ICardVaultDbContext` (a *data*-type dependency the port could not break). Moved services switch from the concrete type to the interface in their constructors — behavior identical. This expands CV-S2+S3 beyond a pure namespace move; the 650-test net still gates it. The original "Contracts stay in Api" (CV-S6) statement is superseded for the coupled contracts.
- **ADR-7 — Pure calculators move to Domain by re-parameterizing with primitives (decided during CV-S1 apply).** The calculators (`CalculateMinimum`, `ComputeAverageDailyBalance`, `ApplyClosingTotals`, `MapResponseCode`) originally took EF entity types as parameters. Moving them to Domain as-is would force `Domain → Infrastructure.Persistence`, creating the cycle `Persistence → Application → Domain → Persistence` (violates ARCH-DEP-1). Options weighed: (a) re-parameterize the pure computational core with primitives (decimals/dates/primitive lists); (b) introduce Domain DTO/value objects as inputs; (c) leave them in services. **Decision: (a).** Domain stays dependency-free and ARCH-2 is genuinely met, without starting the DDD aggregate work deferred to Ola 4+ (rejecting (b)). The tradeoff: calculator **signatures change**, so this is NOT a byte-for-byte move — call sites adapt to pass primitives, and the **numeric result must be provably identical** (the 650-test regression net is the gate). `MapResponseCode(string?)` is already primitive-parameterized and moves directly. This is the one CV-S1 step where "strictly mechanical" does not hold; method *logic* is preserved, only the parameter shape changes.
