...
- TCP ISO 8583 mTLS / IP allowlist (port 7000) — separate track, requires acquirer certs.
- PIN hashing upgrade (BCrypt/Argon2id) — later wave; blocked on data migration + forced reset.
- Full invitation-based self-registration flow — owned by `secure-user-registration`.
- `AllowInvalidCert=true` in IsoSwitch appsettings — flagged P2, not blocked.
- Plaintext `Password=postgres` in connection strings — flagged P2, not blocked.
- Email transport / SMTP configuration — out of scope.
- Frontend CORS configuration or UI admin surface — deferred.
