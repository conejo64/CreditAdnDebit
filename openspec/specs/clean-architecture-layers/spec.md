# Capability Spec: Clean Architecture Layers (CardVault + IsoSwitch)

This specification documents the structural and architectural requirements for the clean architecture layer population in the CardVault and IsoSwitch backend services.

---

## Scope Note

This spec describes behavior-preserving reorganization of code across .NET projects within the solution (`backend/CardSwitchPlatform.sln`). No application behavior changes. No new features. No bug fixes bundled in. Requirements are expressed as:
...
No placeholder file (`Class1.cs`) SHALL remain in `CardVault.Domain`.
...
After the implementation, neither `SwitchTxnConsumer` nor `AuthDecisionPublisher` SHALL exist in the
...
`CardVault.Api` SHALL contain ONLY:
...
`CardVault.Api` SHALL NOT contain any of the following:
...
No placeholder file (`Class1.cs`) SHALL remain in `IsoSwitch.Domain`.
...
None of the above types SHALL remain in `IsoSwitch.Api`'s source tree.
...
`IsoSwitch.Api` SHALL contain ONLY:
...
These two codec stacks SHALL NOT be merged. After implementation:
...
The following MUST remain true throughout implementation and at the end:
...
The following are explicitly NOT requirements of this capability:
